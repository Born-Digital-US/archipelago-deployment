...
build_update:
	...
	git pull
	docker-compose up -d
	mv web/modules/contrib/webform_strawberryfield webform_strawberryfield_backup
	...
	mv webform_strawberryfield_backup web/modules/contrib/webform_strawberryfield
...
build_linux:
	...
	git clone --branch $(WEBFORM_SBF_BRANCH) $(WEBFORM_SBF_REPO) web/modules/contrib/webform_strawberryfield
	docker-compose exec -T -w /var/www/html php bash -c "chown -R :www-data /var/www/html/web"
